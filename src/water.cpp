...
Water::Water(float x, float y, color_t color) {
    ...
    static const GLfloat vertex_buffer_data[] = {
            -(float) INF, 0.0f, -(float) INF, // triangle 1 : begin
            -(float) INF, 0.0f, (float) INF,
            (float) INF, 0.0f, (float) INF, // triangle 1 : end
            (float) INF, 0.0f, (float) INF, // triangle 2 : begin
            -(float) INF, 0.0f, -(float) INF,
            (float) INF, 0.0f, -(float) INF, // triangle 2 : end
    };
...
//            trgls_data[((i+100)/4)*450 + ((j+100)/4)*9 + 8] = (float)j+boat.position.z;
            ...
            trgls_data[((i+400)/16)*450 + ((j+400)/16)*9 + 1] = 0.05;
            trgls_data[((i+400)/16)*450 + ((j+400)/16)*9 + 2] = (float)j;

            trgls_data[((i+400)/16)*450 + ((j+400)/16)*9 + 3] = (float)i;
            trgls_data[((i+400)/16)*450 + ((j+400)/16)*9 + 4] = 0.05;
            trgls_data[((i+400)/16)*450 + ((j+400)/16)*9 + 5] = (float)j+16;

            trgls_data[((i+400)/16)*450 + ((j+400)/16)*9 + 6] = (float)i+16;
            trgls_data[((i+400)/16)*450 + ((j+400)/16)*9 + 7] = 0.05;
            trgls_data[((i+400)/16)*450 + ((j+400)/16)*9 + 8] = (float)j;
        }
    }
    this->object2 = create3DObject(GL_TRIANGLES, 50 * 50 * 3, trgls_data, COLOR_BLUE, GL_FILL);
}

void Water::draw(glm::mat4 VP, Boat boat) {
